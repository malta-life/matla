...
const ALLOWED_ORIGIN_LIST = ALLOWED_ORIGIN
  .split(',')
...
app.use(cors({
  origin(origin, callback) {
    ...
    if (ALLOWED_ORIGIN === '*' || ALLOWED_ORIGIN_LIST.includes('*')) return callback(null, true);
    if (ALLOWED_ORIGIN_LIST.includes(origin)) return callback(null, true);
    return callback(null, false);
  },
  credentials: true,
  methods: ['GET', 'POST', 'PUT', 'PATCH', 'DELETE', 'OPTIONS'],
  allowedHeaders: ['Content-Type', 'Authorization', 'X-App-Role', 'X-App-User', 'X-App-Session']
}));
...
app.post('/api/auth/request-reset', authMiddleware, resetRateLimiter, async (req, res) => {
  try {
    const scope = sanitizeScope(req.body?.scope);
