...
function generateLeadId(existingIds = new Set()) {
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
        ...
        const formatted = `${id.slice(0, 4)}-${id.slice(4)}`;
        if (!existingIds.has(formatted)) return formatted;
    }
    // Fallback: if ID space is exhausted, recycle from unsuccessful leads.
    const reusable = leadRecords
        .filter((lead) => normalizePaymentStatus(lead.paymentStatus) === 'Unsuccessful')
        .map((lead) => String(lead.leadId || '').trim())
        .filter(Boolean)
        .find((id) => !existingIds.has(id));
    if (reusable) return reusable;
    return `LID-${Date.now().toString(36).slice(-8).toUpperCase()}`;
}
...
const STORAGE_KEYS = {
    ...
    ,faAttendance: 'faAttendanceRecords'
    ,uiPrefs: 'uiPrefs'
...
const AUTH_REMEMBER_KEY = 'authRememberedIdentity';
const SESSION_KEYS = {
    adminRole: 'adminRole',
    adminTeam: 'adminTeam',
...
const REMOTE_SYNC_KEYS = [
    ...
    STORAGE_KEYS.faAttendance,
    STORAGE_KEYS.usedUsernames
];

// Session storage
let currentJFA = null;
let jfaRecords = JSON.parse(localStorage.getItem(STORAGE_KEYS.records)) || [];
...
let archivedJfaNames = new Set(JSON.parse(localStorage.getItem(STORAGE_KEYS.archivedJfas) || '[]'));
...
let leadRecords = JSON.parse(localStorage.getItem(STORAGE_KEYS.leads)) || [];
let appointmentRecords = JSON.parse(localStorage.getItem(STORAGE_KEYS.appointments) || '[]');
let faAttendanceRecords = JSON.parse(localStorage.getItem(STORAGE_KEYS.faAttendance) || '[]');
let jfaProfiles = JSON.parse(localStorage.getItem(STORAGE_KEYS.jfaProfiles) || '{}');
let faProfiles = JSON.parse(localStorage.getItem(STORAGE_KEYS.faProfiles) || '{}');
if (!Array.isArray(faAttendanceRecords)) {
    faAttendanceRecords = [];
}
let disabledFacilityKeys = new Set(JSON.parse(localStorage.getItem(STORAGE_KEYS.disabledFacilities) || '[]'));
...
let learningOverrides = JSON.parse(localStorage.getItem(STORAGE_KEYS.learningOverrides) || '{}');
let interventionLog = JSON.parse(localStorage.getItem(STORAGE_KEYS.interventions) || '{}');
let learningQuizRecent = JSON.parse(localStorage.getItem(STORAGE_KEYS.learningQuizRecent) || '{}');
...
let adminPerformanceFilterTeam = 'ALL';
...
let sessionSecurityContext = {
    id: sessionStorage.getItem('sessionId') || '',
    startedAt: Number(sessionStorage.getItem('sessionStartedAt') || 0) || 0
};
...
function logAudit(action, details = '') {
    ...
    pushAuditEventToRemote(action, details);
}

async function pushAuditEventToRemote(action, details = '') {
    if (!isRemoteSyncAllowed()) return;
    const safeAction = String(action || '').trim();
    if (!safeAction) return;
    try {
        await fetch(`${REMOTE_API_BASE}/api/audit/append`, {
            method: 'POST',
            headers: buildRemoteHeaders(),
            body: JSON.stringify({
                scope: REMOTE_STATE_SCOPE,
                action: safeAction.slice(0, 120),
                details: String(details || '').slice(0, 4000),
                source: 'web'
            })
        });
    } catch (_) {
        // Keep UI offline-first; local audit is still persisted.
    }
}
...
async function initializeRemotePersistence() {
    ...
    try {
        const response = await fetch(`${REMOTE_API_BASE}/api/state/load?scope=${encodeURIComponent(REMOTE_STATE_SCOPE)}`, {
            headers: REMOTE_API_TOKEN ? { Authorization: `Bearer ${REMOTE_API_TOKEN}` } : undefined
        });
...
function isRemoteSyncAllowed() {
    try {
        ...
        const appOnFileProtocol = window.location.protocol === 'file:';
        if (appOnFileProtocol && isLocalhost) return true;
...
function getRemoteActorRole() {
    if (adminRole === 'ADMIN') return 'ADMIN';
    if (adminRole === 'TEAM') return 'TEAM';
    return 'PERSONAL';
}

function getRemoteActorName() {
    return formatJfaName(appUser || getLeadIdentity()?.name || 'guest');
}

function rotateSessionSecurityContext() {
    const id = `sess-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 10)}`;
    const startedAt = Date.now();
    sessionSecurityContext = { id, startedAt };
    sessionStorage.setItem('sessionId', id);
    sessionStorage.setItem('sessionStartedAt', String(startedAt));
}

function clearSessionSecurityContext() {
    sessionSecurityContext = { id: '', startedAt: 0 };
    sessionStorage.removeItem('sessionId');
    sessionStorage.removeItem('sessionStartedAt');
}

function enforceAbsoluteSessionLimit() {
    if (!appUser) return;
    const startedAt = Number(sessionSecurityContext.startedAt || 0);
    if (!startedAt) return;
    if ((Date.now() - startedAt) > ABSOLUTE_SESSION_MAX_MS) {
...
function buildRemoteHeaders() {
    ...
    headers['x-app-role'] = getRemoteActorRole();
    headers['x-app-user'] = getRemoteActorName();
    if (sessionSecurityContext.id) {
        headers['x-app-session'] = sessionSecurityContext.id;
    }
    return headers;
}

function getUserAccounts() {
    const raw = JSON.parse(localStorage.getItem(STORAGE_KEYS.userAccounts) || '{}');
...
function normalizeLeaderType(value) {
    const raw = String(value || '').trim().toUpperCase();
    return raw === 'FA' ? 'FA' : 'JFA';
}
...
function ensureDefaultUserAccount() {
    ...
    if (needsPassword) {
        // Defer hashing so legacy stores can be upgraded after first login.
        createPasswordRecord(existing.password || 'admin').then((record) => {
...
function ensureDefaultTeamLeaderAccounts() {
    ...
    CORE_TEAM_LEADER_SEEDS.forEach((seed) => {
        ...
        accounts[key] = {
            ...
            leaderType: normalizeLeaderType(existing.leaderType || 'JFA'),
...
function getRememberedAuthIdentity() {
    try {
        ...
        const mode = parsed.mode === 'admin' ? 'admin' : 'signin';
...
function setRememberedAuthIdentity(identifier, mode = 'signin') {
...
function registerServiceWorker() {
    ...
    window.addEventListener('load', () => {
        navigator.serviceWorker.register('service-worker.js?v=97').then((registration) => {
...
function setAuthMode(mode = 'signin') {
    ...
    if (modeHint) {
        if (authMode === 'admin') {
            modeHint.textContent = 'Admin-only sign in. This opens the standalone Admin (All) page.';
...
function hasSettingsAdminAccess() {
    return adminRole === 'ADMIN' || isAppUserAdmin();
}
...
function renderSettingsPage() {
    ...
    const adminControls = hasSettingsAdminAccess();
    ...
    container.innerHTML = `
        <div class="insight-card" id="teamLeaderAccountCard">
            ...
            <div class="menu-page-actions">
                ...
                <button class="action-btn" type="button" onclick="submitSettingsPasswordChange()">Change My Password</button>
        ...
        <div class="insight-card">
            ...
            <div class="menu-page-actions">
                <button class="action-btn" type="button" onclick="createAdminOnlyAccountFromSettings()">Create Admin Account</button>
            </div>
            ...
            <div class="master-capture-grid">
                <div class="form-group">
                    ...
                    <select id="settingsTeamUser">
                        ${Object.values(getUserAccounts()).map((account) => `<option value="${escapeHtml(account.username || '')}">${escapeHtml(formatJfaName(account.username || ''))}</option>`).join('')}
                    </select>
                </div>
                <div class="form-group">
...
function renderSettingsUsersTable(adminControls) {
    if (!adminControls) {
        return '<p class="page-subtitle">Restricted: Admin only.</p>';
    }
    const accounts = getUserAccounts();
...
function renderSettingsUsersTableSection(adminControls = hasSettingsAdminAccess()) {
...
function filterSettingsUsers(value = '') {
    if (!hasSettingsAdminAccess()) return;
...
async function resetSettingsUserPassword(encodedUsername) {
    ...
    saveUserAccounts(accounts);
    setSettingsError('settingsUserError', '');
    logUserMgmtAction('Password reset', `Account: ${formatJfaName(username)} -> ${resetDefault}`);
...
function lockAppToAuth() {
    ...
    if (userInput) {
        const remembered = getRememberedAuthIdentity();
        userInput.value = remembered?.identifier || '';
        setTimeout(() => userInput.focus(), 60);
    }
    const rememberEl = document.getElementById('authRememberMe');
    if (rememberEl) {
        rememberEl.checked = Boolean(getRememberedAuthIdentity());
    }
...
function getAuthAttemptStorageKeys(scope = 'auth', key = '') {
    const safeScope = String(scope || 'auth').toLowerCase().replace(/[^a-z0-9_-]/g, '');
    const safeKey = normalizeJfaName(key || 'unknown') || 'unknown';
    return {
        failKey: `${safeScope}:loginFail:${safeKey}`,
        lockKey: `${safeScope}:loginLock:${safeKey}`
    };
}

function getAuthLockInfo(scope = 'auth', key = '') {
    const { lockKey } = getAuthAttemptStorageKeys(scope, key);
    const lockedUntil = Number(localStorage.getItem(lockKey) || 0);
    if (lockedUntil && Date.now() < lockedUntil) {
        return {
            locked: true,
            minutesRemaining: Math.max(1, Math.ceil((lockedUntil - Date.now()) / 60000))
        };
    }
    return { locked: false, minutesRemaining: 0 };
}

function registerAuthFailure(scope = 'auth', key = '', maxAttempts = 5, lockMinutes = 10) {
    const { failKey, lockKey } = getAuthAttemptStorageKeys(scope, key);
    const fails = Number(localStorage.getItem(failKey) || 0) + 1;
    localStorage.setItem(failKey, String(fails));
    if (fails >= Math.max(1, Number(maxAttempts) || 5)) {
        const until = Date.now() + Math.max(1, Number(lockMinutes) || 10) * 60 * 1000;
        localStorage.setItem(lockKey, String(until));
        localStorage.removeItem(failKey);
        return { fails, lockedNow: true, lockedUntil: until };
    }
    return { fails, lockedNow: false, lockedUntil: 0 };
}

function clearAuthFailures(scope = 'auth', key = '') {
    const { failKey, lockKey } = getAuthAttemptStorageKeys(scope, key);
    localStorage.removeItem(failKey);
...
async function submitAppAuth() {
    ...
    if (authMode === 'signup') {
        ...
        jfaProfiles[key] = profile;
        saveJfaProfiles();
        if (rememberMe) setRememberedAuthIdentity(username, 'signin');
        else setRememberedAuthIdentity('', 'signin');
    ...
    const lockInfo = getAuthLockInfo('app', resolvedKey);
    if (lockInfo.locked) {
        setAuthUiError(`Account locked. Try again in ${lockInfo.minutesRemaining} minute(s).`);
        return;
    }
    const account = accounts[resolvedKey];
    if (!account || !(await verifyAndMigrateAccountPassword(account, password, accounts, resolvedKey))) {
        const failed = registerAuthFailure('app', resolvedKey, 5, 10);
        if (failed.lockedNow) {
            setAuthUiError('Too many failed attempts. Account locked for 10 minutes.');
        } else {
            setAuthUiError('Invalid username/email or password.');
        }
        passEl.select();
        return;
    }
    ...
    clearAuthFailures('app', resolvedKey);
    ...
    accounts[resolvedKey] = account;
    saveUserAccounts(accounts);
    if (rememberMe) setRememberedAuthIdentity(account.username || username, authMode === 'admin' ? 'admin' : 'signin');
    else setRememberedAuthIdentity('', 'signin');
    if (authMode === 'admin') {
...
function openForgotPasswordModal() {
    ...
    bindForgotPasswordDiagnostics();
    updateForgotPasswordDiagnostics('');
    modal.classList.add('is-open');
    modal.setAttribute('aria-hidden', 'false');
...
async function submitForgotPassword() {
    ...
    if (!emailEl) return;
    const email = (emailEl.value || '').trim().toLowerCase();
    const diag = await updateForgotPasswordDiagnostics(email);
    if (!email || !/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(email)) {
    ...
    if (!diag.accountExists) {
        setModalError('This email is not linked to an account in this scope.', 'forgotPasswordError');
        return;
    }
    if (!diag.smtpReady) {
        setModalError('Reset email server is not ready yet. Try again shortly or contact admin.', 'forgotPasswordError');
        return;
    }
    try {
...
function setForgotDiagChip(id, label, status = 'neutral') {
...
function findLocalAccountByEmail(emailRaw = '') {
    const email = String(emailRaw || '').trim().toLowerCase();
    if (!email) return { exists: false, source: '' };
    const accounts = getUserAccounts();
    const accountHit = Object.entries(accounts).find(([, account]) => String(account?.email || '').trim().toLowerCase() === email);
    if (accountHit) return { exists: true, source: 'accounts', key: accountHit[0] };
    const profileHit = Object.entries(jfaProfiles || {}).find(([, profile]) => String(profile?.email || '').trim().toLowerCase() === email);
    if (profileHit) return { exists: true, source: 'profiles', key: profileHit[0] };
    return { exists: false, source: '' };
}

function bindForgotPasswordDiagnostics() {
    const emailEl = document.getElementById('forgotPasswordEmail');
    if (!emailEl || emailEl.dataset.diagBound === '1') return;
    emailEl.addEventListener('input', () => {
        updateForgotPasswordDiagnostics(emailEl.value || '');
    });
    emailEl.dataset.diagBound = '1';
}

async function updateForgotPasswordDiagnostics(emailRaw = '') {
    const email = String(emailRaw || '').trim().toLowerCase();
    const validEmail = Boolean(email && /^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(email));
    let apiReachable = false;
    let smtpReady = false;
    if (!isRemoteSyncAllowed()) {
        setForgotDiagChip('forgotDiagApi', 'API: unavailable', 'error');
        setForgotDiagChip('forgotDiagSmtp', 'SMTP: unknown', 'warn');
    } else {
        try {
            const response = await fetch(`${REMOTE_API_BASE}/api/auth/config`, {
                headers: buildRemoteHeaders()
            ...
            if (response.ok) {
                const data = await response.json().catch(() => ({}));
                apiReachable = true;
                smtpReady = Boolean(data?.resetEmailConfigured);
                setForgotDiagChip('forgotDiagApi', 'API: reachable', 'ok');
                setForgotDiagChip(
                    'forgotDiagSmtp',
                    smtpReady ? 'SMTP: configured' : 'SMTP: not configured',
                    smtpReady ? 'ok' : 'error'
                );
            } else {
                setForgotDiagChip('forgotDiagApi', 'API: error', 'error');
                setForgotDiagChip('forgotDiagSmtp', 'SMTP: unknown', 'warn');
            }
        } catch (_) {
            setForgotDiagChip('forgotDiagApi', 'API: unreachable', 'error');
            setForgotDiagChip('forgotDiagSmtp', 'SMTP: unknown', 'warn');
        }
    }
    if (!validEmail) {
        setForgotDiagChip('forgotDiagAccount', 'Account: enter valid email', 'neutral');
        return { apiReachable, smtpReady, accountExists: false };
    }
    const accountLookup = findLocalAccountByEmail(email);
    if (accountLookup.exists) {
        setForgotDiagChip('forgotDiagAccount', 'Account: found', 'ok');
    } else {
        setForgotDiagChip('forgotDiagAccount', 'Account: not found in this scope', 'warn');
    }
    return { apiReachable, smtpReady, accountExists: accountLookup.exists };
}
...
async function submitAdminMenuAccess() {
    ...
    const lockInfo = getAuthLockInfo('admin-menu', key);
    if (lockInfo.locked) {
        setModalError(`Too many attempts. Try again in ${lockInfo.minutesRemaining} minute(s).`, 'adminMenuAuthError');
        return;
    }
    ...
    if (!account || !(await verifyAndMigrateAccountPassword(account, password, accounts, key))) {
        const failed = registerAuthFailure('admin-menu', key, 5, 10);
        if (failed.lockedNow) {
            setModalError('Too many failed attempts. Locked for 10 minutes.', 'adminMenuAuthError');
            return;
        }
        setModalError('Invalid username/email or password.', 'adminMenuAuthError');
        return;
    }
    clearAuthFailures('admin-menu', key);
...
async function submitAdminAccess() {
    ...
    const adminKey = normalizeJfaName(username || 'admin');
    const lockInfo = getAuthLockInfo('admin-access', adminKey);
    if (lockInfo.locked) {
        setModalError(`Too many attempts. Try again in ${lockInfo.minutesRemaining} minute(s).`, 'adminAuthError');
        return;
    }
    if (username !== getAdminUsername() || !(await verifyAdminPassword(password))) {
        const failed = registerAuthFailure('admin-access', adminKey, 5, 10);
        if (failed.lockedNow) {
            setModalError('Too many failed attempts. Locked for 10 minutes.', 'adminAuthError');
            return;
        }
    ...
    clearAuthFailures('admin-access', adminKey);
    sessionStorage.setItem(SESSION_KEYS.adminAccess, 'true');
    closeAdminAuthModal();
...
async function submitTeamPortalAccessInline() {
    ...
    const lockInfo = getAuthLockInfo('team-portal-inline', key);
    if (lockInfo.locked) {
        setModalError(`Too many attempts. Try again in ${lockInfo.minutesRemaining} minute(s).`, 'teamInlineError');
        return;
    }
    const account = accounts[key];
    if (!account || !(await verifyAndMigrateAccountPassword(account, password, accounts, key))) {
        const failed = registerAuthFailure('team-portal-inline', key, 5, 10);
        if (failed.lockedNow) {
            setModalError('Too many failed attempts. Locked for 10 minutes.', 'teamInlineError');
            return;
        }
        setModalError('Invalid username/email or password.', 'teamInlineError');
        return;
    }
    clearAuthFailures('team-portal-inline', key);
    if (account.active === false) {
...
async function submitTeamPortalAccess() {
    ...
    const accounts = getUserAccounts();
    const key = resolveAccountKey(identifier) || normalizeJfaName(username);
    const lockInfo = getAuthLockInfo('team-portal', key);
    if (lockInfo.locked) {
        setModalError(`Too many attempts. Try again in ${lockInfo.minutesRemaining} minute(s).`, 'teamAuthError');
        return;
    }
    const account = accounts[key];
    if (!account || !(await verifyAndMigrateAccountPassword(account, password, accounts, key))) {
        const failed = registerAuthFailure('team-portal', key, 5, 10);
        if (failed.lockedNow) {
            setModalError('Too many failed attempts. Locked for 10 minutes.', 'teamAuthError');
            return;
        }
        setModalError('Invalid username/email or password.', 'teamAuthError');
        return;
    }
    clearAuthFailures('team-portal', key);
    if (account.active === false) {
...
async function submitAdminAllAccessInline() {
    ...
    const adminKey = normalizeJfaName(username || 'admin');
    const lockInfo = getAuthLockInfo('admin-all-inline', adminKey);
    if (lockInfo.locked) {
        setModalError(`Too many attempts. Try again in ${lockInfo.minutesRemaining} minute(s).`, 'adminAllInlineError');
        return;
    }
    if (username !== getAdminUsername() || !(await verifyAdminPassword(password))) {
        const failed = registerAuthFailure('admin-all-inline', adminKey, 5, 10);
        if (failed.lockedNow) {
            setModalError('Too many failed attempts. Locked for 10 minutes.', 'adminAllInlineError');
            return;
        }
    ...
    clearAuthFailures('admin-all-inline', adminKey);
    sessionStorage.setItem(SESSION_KEYS.adminAccess, 'true');
...
async function submitAdminAllAccess() {
    ...
    const adminKey = normalizeJfaName(username || 'admin');
    const lockInfo = getAuthLockInfo('admin-all', adminKey);
    if (lockInfo.locked) {
        setModalError(`Too many attempts. Try again in ${lockInfo.minutesRemaining} minute(s).`, 'adminAllAccessError');
        return;
    }
    if (username !== getAdminUsername() || !(await verifyAdminPassword(password))) {
        const failed = registerAuthFailure('admin-all', adminKey, 5, 10);
        if (failed.lockedNow) {
            setModalError('Too many failed attempts. Locked for 10 minutes.', 'adminAllAccessError');
            return;
        }
    ...
    clearAuthFailures('admin-all', adminKey);
    sessionStorage.setItem(SESSION_KEYS.adminAccess, 'true');
    await logAdminAllAccess(username);
    logAudit('Admin (All) access', `${username} opened Admin (All)`);
...
function loadCurrentJfa() {
    ...
    currentJFA = {
        ...
        leadAutoCount: Number.isFinite(stored.leadAutoCount) ? stored.leadAutoCount : getTodayLeadCountForName(stored.name),
        manualConsentAdjustment: Number.isFinite(stored.manualConsentAdjustment) ? stored.manualConsentAdjustment : 0,
        consentForms: stored.consentForms || 0,
...
function getLeadIdentity() {
    if (appUser) {
        ...
        const saved = JSON.parse(localStorage.getItem(STORAGE_KEYS.lastJfaIdentity) || 'null');
        const team = profile.team || saved?.team || '';
        return { name: formatJfaName(appUser), team };
    }
    if (currentJFA?.name) {
...
function getAllTeams() {
    ...
    Object.values(faProfiles || {}).forEach((profile) => {
        if (profile?.team) teams.add(profile.team);
    });
    ...
    return Object.entries(accounts)
        ...
        .map(([key, account]) => {
            ...
            return {
                ...
                leaderType: normalizeLeaderType(account.leaderType || 'JFA'),
...
function getTeamLeaderTeamsByType(type = 'JFA') {
    const normalizedType = normalizeLeaderType(type);
    const teams = new Set();
    getTeamLeaderAccounts().forEach((leader) => {
        if (normalizeLeaderType(leader.leaderType) !== normalizedType) return;
        if (leader.team) teams.add(leader.team);
    });
    if (normalizedType === 'JFA' && !teams.size) {
        DEFAULT_TEAM_NAMES.forEach((team) => teams.add(team));
    }
    return [...teams].filter(Boolean).sort();
}
...
function refreshTeamSelectOptions() {
    ...
    const jfaTeams = getTeamLeaderTeamsByType('JFA');
    const faTeams = getTeamLeaderTeamsByType('FA');
    const jfaTeamSelect = document.getElementById('jfaTeam');
    if (jfaTeamSelect) {
        const current = jfaTeamSelect.value;
        jfaTeamSelect.innerHTML = `<option value="">Select your team</option>${jfaTeams.map((team) => `<option value="${escapeHtml(team)}">${escapeHtml(team)}</option>`).join('')}`;
        if (current && jfaTeams.includes(current)) jfaTeamSelect.value = current;
    }
    const faTeamSelect = document.getElementById('faRegisterTeam');
    if (faTeamSelect) {
        const current = faTeamSelect.value;
        if (faTeams.length) {
            faTeamSelect.innerHTML = `<option value="">Select FA team</option>${faTeams.map((team) => `<option value="${escapeHtml(team)}">${escapeHtml(team)}</option>`).join('')}`;
            faTeamSelect.disabled = false;
            if (current && faTeams.includes(current)) faTeamSelect.value = current;
        } else {
            faTeamSelect.innerHTML = '<option value="">No FA teams created yet</option>';
            faTeamSelect.disabled = true;
        }
    }
    ...
    if (attendanceTeamFilter) {
        ...
        attendanceTeamFilter.innerHTML = `<option value="ALL">All Teams</option>${allTeams.map((team) => `<option value="${escapeHtml(team)}">${escapeHtml(team)}</option>`).join('')}`;
        if (current && (current === 'ALL' || allTeams.includes(current))) attendanceTeamFilter.value = current;
    }
}
...
function confirmClearVisibleNotifications() {
    ...
    const seenAt = getNotificationSeenAt();
    const visible = getApplicableNotifications().filter((note) => {
        if (notificationFilterMode !== 'unread') return true;
        return note.createdAt ? new Date(note.createdAt) > seenAt : false;
    });
...
function goToPage(pageId) {
    ...
    applyMotionContext(activeMenuPage, pageId);
...
function applyLeadSubmissionToDailyStats(name) {
    ...
    if (currentJFA && normalizeJfaName(currentJFA.name) === normalizeJfaName(name) && recordDateKey(currentJFA) === todayKey()) {
        const adjustment = Number.isFinite(currentJFA.manualConsentAdjustment) ? currentJFA.manualConsentAdjustment : 0;
        currentJFA.leadAutoCount = leadAutoCount;
        currentJFA.consentForms = Math.max(0, leadAutoCount + adjustment);
        saveCurrentJfa();
    }
    ...
    if (idx >= 0) {
        ...
        const adjustment = Number.isFinite(record.manualConsentAdjustment)
            ? record.manualConsentAdjustment
            : previous - (Number.isFinite(record.leadAutoCount) ? record.leadAutoCount : 0);
        record.leadAutoCount = leadAutoCount;
        record.manualConsentAdjustment = adjustment;
        record.consentForms = Math.max(0, leadAutoCount + adjustment);
        ...
        if (lastDbd && normalizeJfaName(lastDbd.name) === normalizeJfaName(name) && recordDateKey(lastDbd) === todayKey()) {
            const merged = { ...lastDbd, consentForms: record.consentForms, leadAutoCount, manualConsentAdjustment: adjustment };
...
function syncConsentInputWithAutoCount() {
    ...
    if (!hint) return;
    if (!currentJFA?.name) {
        hint.textContent = 'Auto lead count: 0. You can correct this number if needed.';
        return;
    }
    const leadAutoCount = getTodayLeadCountForName(currentJFA.name);
    const adjustment = Number.isFinite(currentJFA.manualConsentAdjustment) ? currentJFA.manualConsentAdjustment : 0;
    const suggested = Math.max(0, leadAutoCount + adjustment);
    currentJFA.leadAutoCount = leadAutoCount;
    if (input) {
        const currentValue = parseInt(input.value, 10);
        if (Number.isNaN(currentValue) || currentValue === (Number.isFinite(currentJFA.consentForms) ? currentJFA.consentForms : 0)) {
            input.value = String(suggested);
        }
    }
    hint.textContent = `Auto lead count: ${leadAutoCount}. Suggested total: ${suggested}. You can correct this number if needed.`;
}
...
function updateHeroContext(todayRecords, totalForms, totalRegistrations, targetCtx) {
    ...
    const weekForms = weekRecords.reduce((sum, record) => sum + (record.consentForms || 0), 0)
        + getLeadCountForRangeScoped(weekStart, new Date());
...
function setHeroChipState(el, text, tone = 'ok') {
    if (!el) return;
    el.textContent = text;
...
function updateHeroStatusStrip() {
    const strip = document.getElementById('heroStatusStrip');
    if (!strip) return;
    const sessionEl = document.getElementById('heroSessionState');
    const syncEl = document.getElementById('heroSyncState');
    const accessEl = document.getElementById('heroAccessState');
    const hasSession = Boolean(appUser && String(appUser).trim());
    setHeroChipState(sessionEl, hasSession ? 'Session: Active' : 'Session: Locked', hasSession ? 'ok' : 'danger');

    const syncStatus = document.getElementById('offlineStatus');
    const isSyncing = syncStatus?.classList.contains('is-syncing');
    const isOffline = syncStatus?.classList.contains('is-offline') || !navigator.onLine;
    if (isSyncing) {
        setHeroChipState(syncEl, 'Sync: Running', 'warn');
    } else if (isOffline) {
        setHeroChipState(syncEl, 'Sync: Offline', 'danger');
    } else {
        setHeroChipState(syncEl, 'Sync: Online', 'ok');
    }

    if (adminRole === 'ADMIN') {
        setHeroChipState(accessEl, 'Access: Admin', 'ok');
    } else if (adminRole === 'TEAM') {
        setHeroChipState(accessEl, 'Access: Team Leader', 'warn');
    } else {
        setHeroChipState(accessEl, 'Access: User', 'ok');
    }
}
...
function updateHeroSummary() {
    ...
    const totalForms = todayRecords.reduce((sum, r) => sum + (r.consentForms || 0), 0)
        + getLeadCountForRangeScoped(getStartOfDay(new Date()), new Date());
...
function updateHeroTopList() {
    ...
    container.innerHTML = top
        .map((item) => `<span class="hero-top-item">${item.name} (${item.forms})</span>`)
        .join('');
}
...
function setDataFocusMode(pageId, menuPage = '') {
    ...
    const dataMenuPages = new Set(['admin', 'teamDetail', 'archivedJfas', 'leads']);
...
function updateStepperState(activePageId) {
    ...
    document.body.classList.toggle('team-portal-active', activePageId === 'teamDetailPage');
    if (activePageId !== 'teamDetailPage') {
        document.body.classList.remove('team-theme-thando', 'team-theme-thato', 'team-theme-rhulani', 'team-theme-generic');
        document.body.style.removeProperty('--team-hue');
        document.body.style.removeProperty('--team-hue-2');
        document.body.style.removeProperty('--team-headline');
    }
    const order = [
        'landing-page',
        'registration-page',
    ...
    if (stepper) {
        const hideStepper = activePageId === 'admin-standalone-page' || activePageId === 'fa-registration-page';
        stepper.style.display = hideStepper ? 'none' : '';
...
function applyDynamicTeamThemeVars(teamName = '') {
    const root = document.body;
    if (!root) return;
    const hash = hashStringToInt(teamName);
    const hue = hash % 360;
    const hue2 = (hue + 32) % 360;
    const headline = `hsl(${hue} 52% 28%)`;
    root.style.setProperty('--team-hue', String(hue));
    root.style.setProperty('--team-hue-2', String(hue2));
    root.style.setProperty('--team-headline', headline);
}

function applyTeamPortalTheme(teamName = '') {
    const raw = String(teamName || '').toLowerCase();
    const root = document.body;
    if (!root) return;
    root.classList.remove('team-theme-thando', 'team-theme-thato', 'team-theme-rhulani', 'team-theme-generic');
    root.style.removeProperty('--team-hue');
    root.style.removeProperty('--team-hue-2');
    root.style.removeProperty('--team-headline');
    if (!raw) return;
    if (raw.includes('thando')) {
        root.classList.add('team-theme-thando');
        return;
    }
    if (raw.includes('thato')) {
        root.classList.add('team-theme-thato');
        return;
    }
    if (raw.includes('rhulani')) {
        root.classList.add('team-theme-rhulani');
        return;
    }
    root.classList.add('team-theme-generic');
    applyDynamicTeamThemeVars(teamName);
}
...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    if (!hasSessionAccount) {
        ...
        enforceLoggedUserContext();
        if (!sessionSecurityContext.id || !sessionSecurityContext.startedAt) {
            rotateSessionSecurityContext();
        } else {
            enforceAbsoluteSessionLimit();
        }
            ...
            if (this.value === 'PLAN') {
                ...
                if (preAssignedFacilityId) {
                    ...
                    if (autoAssign) {
                        ...
                        document.getElementById('autoAssignedName').textContent = auto.name;
                        document.getElementById('autoAssignedArea').textContent = auto.area ? `Area: ${auto.area}` : '';
                    }
                } else {
...
function openFaRegistrationPage() {
    ...
    const status = document.getElementById('faRegisterStatus');
    ...
    const activeKey = normalizeJfaName(appUser || '');
    const profile = activeKey ? (faProfiles[activeKey] || {}) : {};
    const sourceName = formatJfaName(profile.fullName || '');
    if (sourceName && nameEl && surnameEl) {
        const split = splitJfaName(sourceName);
        nameEl.value = split.name || '';
        surnameEl.value = split.surname || '';
    }
    if (mobileEl) mobileEl.value = profile.phone || '';
    if (emailEl) emailEl.value = profile.email || '';
    if (teamEl && profile.team) {
        teamEl.value = profile.team;
    }

    goToPage('fa-registration-page');
}

function setupFaRegistrationForm() {
    const form = document.getElementById('faRegisterForm');
    ...
    form.addEventListener('submit', (event) => {
        ...
        if (!nameEl || !surnameEl || !mobileEl || !emailEl || !teamEl) return;

        const name = formatJfaName(nameEl.value || '');
        const surname = formatJfaName(surnameEl.value || '');
        const mobile = String(mobileEl.value || '').trim();
        const email = String(emailEl.value || '').trim().toLowerCase();
        const selectedTeam = String(teamEl.value || '').trim();

        if (!name) {
            setFieldError(nameEl, 'Please enter FA name.');
            return;
        }
        if (!surname) {
            setFieldError(surnameEl, 'Please enter FA surname.');
            return;
        }
        if (!isValidPhone(mobile)) {
            setFieldError(mobileEl, 'Please enter a valid mobile number.');
            return;
        }
        if (!email || !isValidEmail(email)) {
        ...
        if (!selectedTeam && !teamEl.disabled) {
            setFieldError(teamEl, 'Please select the FA team.');
            return;
        }

        const fullName = formatJfaName(`${name} ${surname}`.trim());
        const key = normalizeJfaName(fullName);
        const profile = ensureFaProfile(fullName, { username: key, email, phone: mobile, status: 'Active' }) || {};
        profile.name = name;
        profile.surname = surname;
        profile.fullName = fullName;
        profile.phone = mobile;
        profile.email = email;
        profile.team = selectedTeam || profile.team || '';
        ...
        const today = todayKey();
        const alreadyPresent = getFaAttendanceRecordsByDate(today).some((entry) => normalizeJfaName(entry.faKey || entry.faName) === key);
        if (!alreadyPresent) {
            const attendance = sanitizeFaAttendanceRecord({
                id: `faatt-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 8)}`,
                faKey: key,
                faName: fullName,
                team: selectedTeam || profile.team || '',
                dateKey: today,
                timestamp: new Date().toISOString(),
                markedBy: formatJfaName(appUser || getLeadIdentity()?.name || 'System'),
                source: 'fa-register-page'
            });
            if (attendance) {
                faAttendanceRecords = Array.isArray(faAttendanceRecords) ? faAttendanceRecords : [];
                faAttendanceRecords.push(attendance);
                saveFaAttendanceRecords();
            }
        }

        if (statusEl) statusEl.textContent = 'Registration complete.';
        showToast('Registration complete.', 'success');
        logAudit('FA registration complete', `${fullName}${alreadyPresent ? ' (already present)' : ''}`);
        queueUiRefresh();
        setTimeout(() => {
            form.reset();
...
function setupDBDForm() {
    ...
    form.addEventListener('submit', function(e) {
        ...
        const leadAutoCount = getTodayLeadCountForName(currentJFA.name);
        const consentRaw = parseInt(document.getElementById('consentForms').value, 10);
        const consentForms = Number.isNaN(consentRaw) ? 0 : Math.max(0, consentRaw);
        const notes = document.getElementById('notes').value;
        ...
        currentJFA.leadAutoCount = leadAutoCount;
        currentJFA.manualConsentAdjustment = consentForms - leadAutoCount;
        ...
        logAudit('DBD submitted', `${currentJFA.name} • ${currentJFA.team} • ${consentForms} forms`);
    ...
    if (draft) {
        ...
        if (typeof draft.consentForms !== 'undefined') {
            document.getElementById('consentForms').value = draft.consentForms;
        }
        if (draft.notes) {
    ...
    if (consentInput) {
        consentInput.addEventListener('input', () => {
            ...
            const safeEntered = Number.isNaN(entered) ? 0 : Math.max(0, entered);
            const diff = safeEntered - leadAutoCount;
            const sign = diff >= 0 ? '+' : '';
            hint.textContent = `Auto lead count: ${leadAutoCount}. Current entry: ${safeEntered} (manual correction ${sign}${diff}).`;
        });
    }
    syncConsentInputWithAutoCount();
}
...
function navigateToAssignment() {
    ...
    document.getElementById('assignmentDate').textContent = new Date().toLocaleDateString('en-ZA', { 
        weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' 
    });
    document.getElementById('assignedFacilityName').textContent = currentJFA.facility.name;
    document.getElementById('facilityTypeDisplay').textContent = currentJFA.facility.type;
    document.getElementById('facilityAreaDisplay').textContent = currentJFA.facility.area;
    document.getElementById('facilityContactDisplay').textContent = currentJFA.facility.contact;
    document.getElementById('facilityPhoneDisplay').textContent = currentJFA.facility.phone;
    document.getElementById('yourTeamDisplay').textContent = currentJFA.team;
...
function navigateToDBD() {
    ...
    document.getElementById('dbdFacilityName').textContent = currentJFA.facility.name;
    syncConsentInputWithAutoCount();
    goToPage('dbd-page');
}
...
async function resetToLanding() {
    if (!(await confirmLeaveIfDirty())) return;
    currentJFA = null;
    document.getElementById('jfaForm').reset();
    document.getElementById('dbdForm').reset();
    updateResumeBanner();
...
function editLastDbd() {
    ...
    document.getElementById('consentForms').value = last.consentForms || 0;
...
function clearBodyClassesByPrefix(prefix = '') {
    ...
    document.body.classList.forEach((className) => {
        if (className.startsWith(prefix)) removals.push(className);
    });
    removals.forEach((className) => document.body.classList.remove(className));
}

function applyMotionContext(menuPage = '', pageId = '') {
    clearBodyClassesByPrefix('menu-motion-');
    clearBodyClassesByPrefix('page-motion-');
    if (menuPage) {
        const safeMenu = String(menuPage || '').trim().toLowerCase().replace(/[^a-z0-9-]/g, '');
        if (safeMenu) document.body.classList.add(`menu-motion-${safeMenu}`);
    }
    if (pageId) {
        const normalized = String(pageId || '').toLowerCase().replace(/-page$/, '').replace(/[^a-z0-9-]/g, '');
        if (normalized) document.body.classList.add(`page-motion-${normalized}`);
    }
}
...
function openMenuPage(page, pushHistory = true, skipAdminGate = false) {
    ...
    applyMotionContext(page, lastActivePageId);
...
function closeMenuPage(clearHistory = true) {
    ...
    setDataFocusMode(document.querySelector('.page.active')?.id || lastActivePageId, '');
    applyMotionContext('', document.querySelector('.page.active')?.id || lastActivePageId);
...
function setupLeadForm() {
    ...
    form.addEventListener('submit', async (e) => {
        ...
        const entry = {
            ...
            failureReason: '',
            commissionDate: '',
            leadId: generateLeadId(new Set(leadRecords.map((lead) => lead.leadId).filter(Boolean))),
        ...
        showToast(`Lead submitted. Geo: ${geoSummary.statusLabel}.`, 'success');
        renderMyLeads();
...
function renderMyLeads() {
    ...
    breakdownEl.innerHTML = `
        ...
        <p class="page-subtitle">Client personal details are intentionally hidden in this view.</p>
    `;
}
...
function adjustMyConsentFormsToday() {
    ...
    record.leadAutoCount = leadAutoCount;
    record.manualConsentAdjustment = next - leadAutoCount;
...
function sanitizeFaAttendanceRecord(record) {
    if (!record || typeof record !== 'object') return null;
    const name = formatJfaName(record.faName || '');
    const faKey = normalizeJfaName(record.faKey || name);
    const dateKey = String(record.dateKey || '').trim();
    if (!name || !dateKey) return null;
    return {
        id: String(record.id || `faatt-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 8)}`),
        faKey,
        faName: name,
        team: String(record.team || '').trim(),
        dateKey,
        timestamp: record.timestamp || new Date().toISOString(),
        markedBy: formatJfaName(record.markedBy || ''),
        source: String(record.source || 'manual').trim()
    };
}

function getFaAttendanceRecordsByDate(dateKey = todayKey()) {
    const targetDate = String(dateKey || '').trim() || todayKey();
    const clean = [];
    const dedupe = new Set();
    (Array.isArray(faAttendanceRecords) ? faAttendanceRecords : []).forEach((entry) => {
        const safe = sanitizeFaAttendanceRecord(entry);
        if (!safe || safe.dateKey !== targetDate) return;
        const uniqueKey = `${safe.dateKey}::${safe.faKey || normalizeJfaName(safe.faName)}`;
        if (dedupe.has(uniqueKey)) return;
        dedupe.add(uniqueKey);
        clean.push(safe);
    });
    clean.sort((a, b) => (a.faName || '').localeCompare(b.faName || ''));
    return clean;
}

function saveFaAttendanceRecords() {
    scheduleStorageWrite(STORAGE_KEYS.faAttendance, JSON.stringify(Array.isArray(faAttendanceRecords) ? faAttendanceRecords : []));
}

function populateFaAttendanceSelect() {
    const select = document.getElementById('faAttendanceSelect');
    if (!select) return;
    const fas = getFaRegisterDirectory().filter((entry) => entry.active !== false);
    const options = ['<option value="">Select FA...</option>'];
    fas.forEach((entry) => {
        options.push(`<option value="${escapeHtml(entry.key)}">${escapeHtml(entry.name)}${entry.team ? ` (${escapeHtml(entry.team)})` : ''}</option>`);
    });
    select.innerHTML = options.join('');
    const currentKey = normalizeJfaName(appUser || '');
    if (currentKey && fas.some((entry) => entry.key === currentKey)) {
        select.value = currentKey;
    }
}

function openFaAttendanceModal() {
...
function submitFaAttendance() {
    const select = document.getElementById('faAttendanceSelect');
    const manualNameInput = document.getElementById('faAttendanceName');
    if (!select || !manualNameInput) return;
    const selectedKey = String(select.value || '').trim();
    const manualName = formatJfaName(manualNameInput.value || '');
    const faDirectory = getFaRegisterDirectory();
    const selectedEntry = selectedKey ? faDirectory.find((entry) => entry.key === selectedKey) : null;
    const faName = selectedEntry?.name || manualName;
    const faKey = selectedEntry?.key || normalizeJfaName(faName);
    const faTeam = selectedEntry?.team || (faProfiles[faKey]?.team || '');
    if (!faName) {
        showToast('Select an FA or enter the FA full name.', 'error');
        if (select) select.focus();
        return;
    }
    const dayKey = todayKey();
    const exists = (Array.isArray(faAttendanceRecords) ? faAttendanceRecords : []).some((entry) => {
        const safe = sanitizeFaAttendanceRecord(entry);
        if (!safe) return false;
        return safe.dateKey === dayKey && normalizeJfaName(safe.faKey || safe.faName) === normalizeJfaName(faKey || faName);
    });
    if (exists) {
        showToast(`${faName} is already marked present today.`, 'success');
        closeFaAttendanceModal();
        return;
    }
    const marker = formatJfaName(appUser || getLeadIdentity()?.name || 'System');
    const record = sanitizeFaAttendanceRecord({
        id: `faatt-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 8)}`,
        faKey,
        faName,
        team: faTeam,
        ...
        timestamp: new Date().toISOString(),
        markedBy: marker,
        source: 'hero-fa-register'
    });
    if (!record) {
        showToast('Unable to capture FA attendance.', 'error');
        return;
    }
    faAttendanceRecords = Array.isArray(faAttendanceRecords) ? faAttendanceRecords : [];
    faAttendanceRecords.push(record);
    saveFaAttendanceRecords();
    logAudit('FA marked present', `${record.faName} (${record.team || 'No Team'})`);
    closeFaAttendanceModal();
    if (isAdminStandaloneActive() && adminRole === 'ADMIN' && adminMasterTab === 'fas') {
        renderAdminMasterLogger();
    }
    queueUiRefresh();
    showToast(`${record.faName} marked present for today.`, 'success');
}
...
function renderMasterFaRegister() {
    ...
    const presentToday = getFaAttendanceRecordsByDate(todayKey());
    return `
        <div class="insight-card">
            ...
            <div class="insight-list">
                ...
                <span class="insight-chip">Present Today: ${presentToday.length}</span>
            </div>
            <div class="insight-card" style="margin-top:12px;">
                <h4>Present FAs (Today)</h4>
                ${presentToday.length ? `
                    <div class="mini-table">
                        <table>
                            <thead>
                            ...
                            <tbody>
                                ${presentToday.map((entry) => `
                                    <tr>
                                        <td>${escapeHtml(entry.faName || '—')}</td>
                                        <td>${escapeHtml(entry.team || '—')}</td>
                                        <td>${entry.timestamp ? new Date(entry.timestamp).toLocaleTimeString('en-ZA', { hour: '2-digit', minute: '2-digit' }) : '—'}</td>
                                        <td>${escapeHtml(entry.markedBy || 'System')}</td>
                                    </tr>
                                `).join('')}
                            </tbody>
                        </table>
                    ...
                    <div class="menu-page-actions" style="margin-top:10px;">
                        <button class="action-btn secondary" type="button" onclick="exportTodayFaAttendance()">Download Today Attendance</button>
                    </div>
                ` : '<p class="page-subtitle">No FAs marked present yet today.</p>'}
            </div>
            ${fas.length ? `
...
function exportTodayFaAttendance() {
    if (adminRole !== 'ADMIN') return;
    const rows = getFaAttendanceRecordsByDate(todayKey()).map((entry) => ([
        entry.faName || '',
        entry.team || '',
    ...
    const headers = ['FA Name', 'Team', 'Date', 'Marked At', 'Marked By'];
    exportCsvWithHeaders(headers, rows, `fa-attendance-${todayKey()}.csv`);
    logAudit('Exported FA attendance', `Rows: ${rows.length}`);
    logExportHistory('FA Attendance Export', rows.length);
}
...
function renderMasterClientsRegistry() {
    ...
    const recentClients = uniqueClients.slice(0, 8);
    return `
        <div class="insight-card">
            <h4>Clients</h4>
            <p class="page-subtitle">Admin (All) can view clients who recently joined and existing clients. Full structured list rollout is available soon.</p>
            <div class="insight-list">
                <span class="insight-chip">Unique Clients Captured: ${uniqueClients.length}</span>
                <span class="insight-chip">Recent Clients: ${recentClients.length}</span>
                <span class="insight-chip">Status: Full list available soon</span>
            </div>
...
function buildMonthlyPerformanceReviewRows(monthlyTarget = 20) {
    const now = new Date();
    const monthStart = getStartOfMonth(now);
    const monthRecords = getScopedRecordsByRange(monthStart, now);
    const monthLeads = getScopedLeadsInRange(monthStart, now);
    const baseDirectory = getCompanyJfaDirectory();
    const byKey = new Map();

    baseDirectory.forEach((entry) => {
        const key = normalizeJfaName(entry.name || entry.key || '');
        if (!key || isTeamLeaderName(entry.name, entry.team || '')) return;
        byKey.set(key, {
            key,
            name: formatJfaName(entry.name || key),
            team: entry.team || '',
            forms: 0,
            leads: 0
        });
    ...
    monthRecords.forEach((record) => {
        const key = normalizeJfaName(record?.name || '');
        if (!key || isTeamLeaderName(record?.name || '', record?.team || '')) return;
        const current = byKey.get(key) || {
            key,
            name: formatJfaName(record?.name || key),
            team: record?.team || '',
            forms: 0,
            leads: 0
        };
    ...
    monthLeads.forEach((lead) => {
        const key = normalizeJfaName(lead?.jfaName || '');
        if (!key || isTeamLeaderName(lead?.jfaName || '', lead?.jfaTeam || '')) return;
        const current = byKey.get(key) || {
            key,
            name: formatJfaName(lead?.jfaName || key),
            team: lead?.jfaTeam || '',
            forms: 0,
            leads: 0
        };
    ...
    return [...byKey.values()]
        .map((item) => {
            const total = Number(item.forms || 0) + Number(item.leads || 0);
            return {
                ...item,
                total,
                target: monthlyTarget,
                gap: Math.max(0, monthlyTarget - total),
                underperforming: total < monthlyTarget
            };
        })
        .sort((a, b) => {
...
function setAdminPerformanceFilterTeam(value = 'ALL') {
    adminPerformanceFilterTeam = String(value || 'ALL').trim() || 'ALL';
    renderAdminMasterLogger();
}

function exportMasterPerformanceReviewCsv() {
    if (adminRole !== 'ADMIN') return;
    const monthlyTarget = 20;
    const rows = buildMonthlyPerformanceReviewRows(monthlyTarget)
        .filter((entry) => entry.underperforming)
        .filter((entry) => adminPerformanceFilterTeam === 'ALL' || entry.team === adminPerformanceFilterTeam)
        .map((entry) => [
            entry.name,
            entry.team || 'No Team',
            entry.forms,
    ...
    exportCsvWithHeaders(headers, rows, `performance-review-${todayKey()}.csv`);
    logAudit('Exported Performance Review', `Rows: ${rows.length} • Filter: ${adminPerformanceFilterTeam}`);
    logExportHistory('Performance Review Export', rows.length);
}

function renderMasterPerformanceReview() {
    const monthlyTarget = 20;
    const rows = buildMonthlyPerformanceReviewRows(monthlyTarget);
    const underperformers = rows.filter((entry) => entry.underperforming);
    const teamSet = new Set(rows.map((entry) => entry.team).filter(Boolean));
    if (adminPerformanceFilterTeam !== 'ALL' && !teamSet.has(adminPerformanceFilterTeam)) {
        adminPerformanceFilterTeam = 'ALL';
    }
    const filtered = underperformers.filter((entry) => adminPerformanceFilterTeam === 'ALL' || entry.team === adminPerformanceFilterTeam);
    const avgTotal = rows.length
        ? Math.round((rows.reduce((sum, entry) => sum + entry.total, 0) / rows.length) * 10) / 10
        : 0;
    const monthLabel = new Date().toLocaleString('en-ZA', { month: 'long', year: 'numeric' });
    const underperformerNames = filtered.map((entry) => entry.name);
    return `
        <div class="insight-card">
            <h4>Performance Review</h4>
            <p class="page-subtitle">Monthly underperformance list. Any JFA with less than ${monthlyTarget} total submissions (consent forms + leads) is flagged.</p>
            <div class="insight-list">
                <span class="insight-chip">Month: ${escapeHtml(monthLabel)}</span>
                <span class="insight-chip ${underperformers.length ? 'danger' : ''}">Underperforming: ${underperformers.length}</span>
                <span class="insight-chip">Average Output: ${avgTotal}</span>
                <span class="insight-chip">Target: ${monthlyTarget}</span>
            </div>
            <div class="menu-page-actions" style="margin-top:10px;">
                <select id="adminPerformanceTeamFilter" onchange="setAdminPerformanceFilterTeam(this.value)">
                    <option value="ALL" ${adminPerformanceFilterTeam === 'ALL' ? 'selected' : ''}>All Teams</option>
                    ${[...teamSet].sort((a, b) => a.localeCompare(b)).map((team) => `
                        <option value="${escapeHtml(team)}" ${adminPerformanceFilterTeam === team ? 'selected' : ''}>${escapeHtml(team)}</option>
                    `).join('')}
                </select>
                <button class="action-btn secondary" type="button" onclick="exportMasterPerformanceReviewCsv()">Download Review CSV</button>
            </div>
            <div class="insight-card" style="margin-top:12px;">
                <h4>Underperforming JFAs (Not Meeting Target)</h4>
                <p class="page-subtitle">Target is ${monthlyTarget} submissions for the month. Names below are currently under target.</p>
                ${underperformerNames.length ? `
                    <div class="insight-list">
                        ${underperformerNames.map((name) => `
                            <span class="insight-chip danger">${escapeHtml(name)}</span>
                        `).join('')}
                    </div>
                ` : `
                    <p class="page-subtitle">No JFAs are underperforming for this filter.</p>
                `}
            </div>
            ${filtered.length ? `
                <div class="mini-table" style="margin-top:12px;">
                    <table>
                        ...
                        <tbody>
                            ${filtered.map((entry) => `
                                <tr>
                                    <td>${escapeHtml(entry.name)}</td>
                                    <td>${escapeHtml(entry.team || 'No Team')}</td>
                                    <td>${entry.forms}</td>
                                    <td>${entry.leads}</td>
                                    <td>${entry.total}</td>
                                    <td>${entry.gap}</td>
                                    <td>
                                        <button class="action-btn secondary" type="button" onclick="openAdminJfaDetail('${escapeHtml(entry.name)}','admin')">Open Profile</button>
                                    </td>
                                </tr>
                            `).join('')}
                        </tbody>
            ...
            ` : `
                <p class="page-subtitle" style="margin-top:10px;">No underperforming JFAs for this filter.</p>
            `}
        </div>
    `;
}
    ...
    return `
        <div class="insight-card">
            ...
            <div class="master-capture-grid">
                ...
                <div class="form-group">
                    <label for="masterLeaderType">Team Leader Type</label>
                    <select id="masterLeaderType">
                        <option value="JFA">JFA Team Leader</option>
                        <option value="FA">FA Team Leader</option>
                    </select>
                </div>
                <div class="form-group">
                ...
                <table>
                    ...
                    <tbody>
                        ${leaders.length ? leaders.map((leader) => {
                            ...
                            return `
                            <tr>
                                <td><input type="text" id="mtl-name-${encoded}" value="${escapeHtml(leader.username)}"></td>
                                <td>
                                    <select id="mtl-type-${encoded}">
                                        <option value="JFA" ${normalizeLeaderType(leader.leaderType) === 'JFA' ? 'selected' : ''}>JFA</option>
                                        <option value="FA" ${normalizeLeaderType(leader.leaderType) === 'FA' ? 'selected' : ''}>FA</option>
                                    </select>
                                </td>
                                <td><input type="text" id="mtl-team-${encoded}" value="${escapeHtml(leader.team || '')}"></td>
                                <td><input type="email" id="mtl-email-${encoded}" value="${escapeHtml(leader.email || '')}"></td>
                                <td>
                        ...
                        `;
                        }).join('') : '<tr><td colspan="5">No team leaders configured yet.</td></tr>'}
                    </tbody>
                </table>
            </div>
        </div>
...
async function createMasterTeamLeader() {
    ...
    const typeEl = document.getElementById('masterLeaderType');
    ...
    if (!teamEl || !typeEl || !nameEl || !emailEl || !passEl) return;
    const team = String(teamEl.value || '').trim();
    const leaderType = normalizeLeaderType(typeEl.value || 'JFA');
    ...
    jfaProfiles[key] = profile;
    saveJfaProfiles();
    logUserMgmtAction('Team leader created', `${username} • ${leaderType} • ${team}`);
    renderTeamPortalButtons();
    refreshTeamSelectOptions();
    renderAdminMasterLogger();
...
function saveMasterTeamLeader(keyRaw) {
    ...
    const typeEl = document.getElementById(`mtl-type-${encoded}`);
    ...
    saveJfaProfiles();
    logUserMgmtAction('Team leader updated', `${formatJfaName(account.username || nextKey)} • ${leaderType} • ${team}`);
    renderTeamPortalButtons();
    refreshTeamSelectOptions();
    renderAdminMasterLogger();
...
function renderMasterSubmittedLeads() {
    ...
    const presentFas = getFaAttendanceRecordsByDate(todayKey());
    const availableFaEntries = presentFas.length
        ? presentFas.map((entry) => ({
            key: entry.faKey || normalizeJfaName(entry.faName || ''),
            label: `${entry.faName || 'Unnamed FA'}${entry.team ? ` (${entry.team})` : ''}`
        }))
        : getFaRegisterDirectory()
            .filter((entry) => entry.active)
            .map((entry) => ({
                key: entry.key,
                label: `${entry.name}${entry.team ? ` (${entry.team})` : ''}`
            }));
    ...
    return `
        <div class="insight-card">
            ...
            <div class="menu-page-actions" style="margin-top:12px;">
                ...
                <select id="allocateFaSelect" ${availableFaEntries.length ? '' : 'disabled'}>
                    ${availableFaEntries.length
                        ? availableFaEntries.map((entry) => `<option value="${escapeHtml(entry.key)}">${escapeHtml(entry.label)}</option>`).join('')
                        : '<option>No available FAs yet</option>'}
                </select>
                <button class="action-btn secondary" type="button" ${availableFaEntries.length ? '' : 'disabled'}>Allocate</button>
            </div>
        </div>
    `;
}
...
function renderMasterFacilitiesEditor() {
    const list = [...facilities].sort((a, b) => (a.name || '').localeCompare(b.name || '', 'en', { sensitivity: 'base' }));
            ...
            <div class="mini-table">
                <table>
                    ...
                    <tbody>
                        ${list.map((facility) => `
                            <tr>
                                <td><input type="text" id="mf-name-${facility.id}" value="${escapeHtml(facility.name || '')}"></td>
                                <td><input type="text" id="mf-type-${facility.id}" value="${escapeHtml(facility.type || 'Other')}"></td>
                                <td><input type="text" id="mf-area-${facility.id}" value="${escapeHtml(facility.area || '')}"></td>
                                <td><input type="text" id="mf-contact-${facility.id}" value="${escapeHtml(facility.contact || 'Receptionist')}"></td>
                                <td><input type="text" id="mf-phone-${facility.id}" value="${escapeHtml(facility.phone || 'N/A')}"></td>
                                <td>
                                    <div class="facility-inline-actions">
...
function openAdminJfaDetail(name, source = 'admin', forceOpen = false) {
    ...
    bodyEl.innerHTML = `
        ...
        <div class="insight-card">
            ...
            <p class="page-subtitle">Only authorized team leaders/admin can adjust today's confirmed count.</p>
            <div class="menu-page-actions">
                <input type="number" id="${inputId}" min="0" value="${todayForms}" ${canEdit ? '' : 'disabled'}>
                <button class="action-btn" type="button" onclick="confirmConsentForms('${escapeHtml(name)}','${inputId}','${source}')" ${canEdit ? '' : 'disabled'}>Verify Consent Forms</button>
            </div>
        </div>
        ${isTeamLeader ? '' : `
        <div class="insight-card">
...
function confirmConsentForms(name, inputId = 'adminConfirmFormsAdmin', source = 'admin') {
    ...
    todayRecords.forEach((record, index) => {
        record.consentForms = index === 0 ? newValue : 0;
    });
    ...
    scheduleStorageWrite(STORAGE_KEYS.records, JSON.stringify(jfaRecords));
    logAudit('Verified consent forms', `${formatJfaName(name)}: ${newValue}`);
    showToast('Consent forms verified and updated.', 'success');
    renderAdminTeamDetails();
    if (source === 'team') {
        openAdminJfaDetail(name, 'team', true);
    } else {
...
function resetAllData() {
    ...
    localStorage.removeItem(STORAGE_KEYS.faAttendance);
    ...
    scheduleStorageWrite(STORAGE_KEYS.faProfiles, JSON.stringify(faProfiles));
    scheduleStorageWrite(STORAGE_KEYS.faAttendance, JSON.stringify(faAttendanceRecords));
...
function buildBrandedCsv(reportLabel, headers, rows) {
...
function exportCsvWithHeaders(headers, rows, filename, reportLabel = 'Data Export') {
    const content = buildBrandedCsv(reportLabel, headers, rows);
...
function exportPdfWithHeaders(headers, rows, filename, reportLabel = 'Data Export') {
    ...
    const logoSrc = new URL(EXPORT_LOGO_FILE, window.location.href).href;
    const generatedAt = new Date().toLocaleString('en-ZA');
    const exportedBy = formatJfaName(appUser || 'Unknown user');
...
<body>
  <div class="report-shell">
    <header class="report-header">
      ...
      <div class="report-meta">Generated: ${escapeHtml(generatedAt)}<br>Exported by: ${escapeHtml(exportedBy)}<br>Total rows: ${safeRows.length}</div>
      <img class="report-logo" src="${escapeHtml(logoSrc)}" alt="Matla Life logo">
    </header>
    ${safeRows.length ? `<table><thead><tr>${tableHead}</tr></thead><tbody>${tableBody}</tbody></table>` : '<p class="empty">No records available for this export.</p>'}
  </div>
...
function updateStatistics() {
    ...
    const monthLeadTotals = new Map();
    const monthLeads = getScopedLeadsInRange(monthStart, now);
    monthLeads.forEach((lead) => {
        const key = normalizeJfaName(lead.jfaName);
        if (!key) return;
        const entry = monthLeadTotals.get(key) || { name: formatJfaName(lead.jfaName), count: 0 };
        entry.count += 1;
        monthLeadTotals.set(key, entry);
    });
    monthTotals.forEach((entry, key) => {
        const leadEntry = monthLeadTotals.get(key);
        if (leadEntry) entry.forms += leadEntry.count;
    });
    monthLeadTotals.forEach((entry, key) => {
        if (!monthTotals.has(key)) {
            monthTotals.set(key, { name: entry.name, forms: entry.count });
        }
    });
...
function buildTeamRoster(teamName) {
    ...
    return [...roster.values()].map((entry) => {
        ...
        const formsToday = monthMine.filter((record) => recordDateKey(record) === today).reduce((sum, record) => sum + (record.consentForms || 0), 0);
        ...
        const digitalToday = getLeadCountForRange(getStartOfDay(now), now, key, teamName);
        const digitalMonth = getLeadCountForRange(monthStart, now, key, teamName);
        const digitalAll = getLeadCountForRange(new Date(2000, 0, 1), now, key, teamName);
        ...
        const presentToday = (formsToday + digitalToday) > 0;
        ...
        const perf = getPerformanceBand(formsToday + digitalToday, formsMonth + digitalMonth);
        const needsAssistance = !isTeamLeader && perf.code === 'under' && underStreak >= 3;
        ...
        return {
            ...entry,
            presentToday,
            formsToday: formsToday + digitalToday,
            formsMonth: formsMonth + digitalMonth,
            formsAll: formsAll + digitalAll,
...
function updateTargetsAndLeaders(records) {
    ...
    const dailyTotal = daily.reduce((sum, r) => sum + (r.consentForms || 0), 0) + getLeadCountForRangeScoped(todayStart, end);
    const weeklyTotal = weekly.reduce((sum, r) => sum + (r.consentForms || 0), 0) + getLeadCountForRangeScoped(weekStart, end);
    const monthlyTotal = monthly.reduce((sum, r) => sum + (r.consentForms || 0), 0) + getLeadCountForRangeScoped(monthStart, end);
    ...
    const appendLeadCounts = (recordsSet, leadsSet) => {
        if (!leadsSet.length) return recordsSet;
        const output = [...recordsSet];
        const leadTotals = new Map();
        leadsSet.forEach((lead) => {
            const key = normalizeJfaName(lead.jfaName);
            if (!key) return;
            const team = lead.jfaTeam || jfaProfiles[key]?.team || '';
            const existing = leadTotals.get(key) || { name: formatJfaName(lead.jfaName), team, consentForms: 0 };
            existing.consentForms += 1;
            if (!existing.team && team) existing.team = team;
            leadTotals.set(key, existing);
        });
        leadTotals.forEach((entry) => output.push(entry));
        return output;
    };

    const dailyWithLeads = appendLeadCounts(daily, dailyLeads);
    const weeklyWithLeads = appendLeadCounts(weekly, weeklyLeads);
    const monthlyWithLeads = appendLeadCounts(monthly, monthlyLeads);
